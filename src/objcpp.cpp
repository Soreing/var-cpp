...
size_t writeArray_t(array &arr, char* buff, const size_t size)
{
    ...
    {   elmBytes = writeJCON(arr[i], buff + totBytes, size - totBytes);
        totBytes += elmBytes;
    }
...
size_t writeAttrib_t(atr &att, char* buff, const size_t size)
{
    ...
    valueBytes = writeJCON(att.val, buff + nameLength, size - nameLength);
...
{
    switch(val.type)
    {   case boolean_t:   return writeBoolean_t(*(bool*)val.data, buff, size);
        case integer_t:   return writeInteger_t(*(int*)val.data, buff, size);
        case real_t:      return writeReal_t(*(double*)val.data, buff, size);
        case text_t:      return writeText_t(*(str*)val.data, buff, size);
        case array_t:     return writeArray_t(*(array*)val.data, buff, size);
        case object_t:    return writeObject_t(*(vec<atr>*)val.data, buff, size);
...
{
    var res = array{};
    array* arr = (array*)res.data;
    arr->resize(elements);

    size_t totUsed=0;
    size_t elmUsed=0;
    for(size_t i = 0; i < elements; i++)
    {   (*arr)[i] = readJCON(buff+totUsed, size-totUsed, &elmUsed);
        totUsed += elmUsed;
    }
...
size_t readAttrib_t(atr &att, char* buff, const size_t size)
{
    ...
    att.val = readJCON(buff+nameLen, size-nameLen, &dataSize);
...
var readJCON(char* buff, const size_t size, size_t* bytes)
{
    varType type;
    size_t byteCount;
    size_t elemCount;
    ...
    //std::cout << "TYPE "<< type <<"\n";
    //std::cout<< "HEADER SIZE: " << headerSize << "BYTE COUNT: " <<byteCount <<"\n";
